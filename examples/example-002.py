...
if __name__ == '__main__':
  ...
  if writeCsv:
    ...
    f.write('{:.0f}'.format(saturdayFvec[0]))
    for i in xrange(1,len(saturdayFvec)):
      f.write("\n")
      f.write('{:.0f}'.format(saturdayFvec[i]))
    f.close()

    f = open("tuesdayFvec.csv","w")
    f.write('{:.0f}'.format(tuesdayFvec[0]))
    for i in xrange(1,len(tuesdayFvec)):
      f.write("\n")
      f.write('{:.0f}'.format(tuesdayFvec[i]))
    f.close()

    f = open("labMTvec.csv","w")
    f.write('{:.8f}'.format(labMTvector[0]))
    for i in xrange(1,len(labMTvector)):
      f.write("\n")
      f.write('{:.8f}'.format(labMTvector[i]))
    f.close()
